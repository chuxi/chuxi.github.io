...
<!-- 并发在目前的编程概念里可以划分为两种层面，进程与线程层面。最直接的实现方式是在操作系统层面使用进程，彼此之间完全独立，但显然因为资源，上下文切换等问题而受到限制，另一种则是像Java所使用的这种并发系统，共享内存和IO这样的资源，但此时也就不可避免资源竞争的问题。
